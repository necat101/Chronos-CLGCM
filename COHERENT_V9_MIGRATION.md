...
This retains the broad direction of the previous run while using a one-token
response floor for this dataset. Its pinned revision contains 777,420 valid
responses shorter than 16 GPT-2 tokens (about 14.8% of all rows), so a floor of
16 would discard a material part of the corpus and fail the default zero-reject
audit. Raise the floor only when a reviewed cache audit justifies that filtering.
The writer auxiliary adds no parameters and samples only one in
