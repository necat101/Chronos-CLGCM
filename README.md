# Chronos v0.6.2 (alpha): A Hybrid Memory-Reasoning Architecture

A novel AI architecture that synergistically integrates Google's Titans memory system with a Hierarchical Reasoning Model (HRM) to move beyond the limitations of scale and take a decisive step on the path to AGI.

-----

### 📢 **Minor Update in v0.6.2: Optimized Large Dataset Handling**

> Building upon v0.6, this version refines the pre-chunked dataset workflow for **even greater efficiency** when dealing with massive datasets.
>
> 1.  **Consolidated Chunk Files:** The `dataset_chunk_create.py` script now saves **multiple processed chunks into larger, consolidated `.pt` files**. This significantly reduces the number of individual files, making data management and transfer much easier.
> 2.  **Manifest-Driven Loading with Caching:** The `--pre_pt_dataset` loading mechanism now uses a `manifest.jsonl` file (generated by the chunker) to locate specific chunks within the consolidated files. It also implements **caching** to avoid repeatedly loading the same large `.pt` file when accessing consecutive chunks, optimizing I/O during training.
> 3.  v0.6.2: Migrated from "keyboard" to "signal" library for chat interruption
...
If your dataset (`.jsonl` format, Kayla-style recommended) is too large to fit comfortably in RAM during training startup, use this script first. It analyzes the dataset, determines an optimal fixed sequence length, and then chunks each sample into multiple overlapping, padded tensor dictionaries. These dictionaries are then **consolidated** (e.g., 1000 chunks per file) into larger `.pt` files for easier management.
...
1.  **Run the Chunking Script:**
    ...
    python dataset_chunk_create.py \
        ...
        --chunks-per-file 1000 `# Number of chunks per .pt file`
    ```
    ...
    * Finally, it processes the dataset, creating consolidated `.pt` files (e.g., `consolidated_chunks_0000000.pt`, `consolidated_chunks_0000001.pt`, ...) and a `manifest.jsonl` file in the `--output-dir`. The manifest maps each logical chunk to its specific file and index within that file.

2.  **Use the Chunked Data for Training:**

    When running `chronos.py train` or `finetune`, use the `--pre_pt_dataset` flag, point `--train` to the *output directory* containing the consolidated `.pt` files and the manifest, and **specify the `MAX_SEQ_LENGTH`** noted from the chunker script's output using the `--max_length` argument.

    ```bash
    python chronos.py train \
        --pre_pt_dataset `# Enable loading from consolidated .pt files via manifest` \
        --train "./my_large_data_chunked_tensors" `# Directory containing .pt files + manifest.jsonl` \
    ...
    > * The `--pre_pt_dataset` loader uses **caching** to efficiently read from the consolidated files.
...
2.  **Fine-Tuning (LoRA):** Adapt a pre-trained model using LoRA. Can use standard or pre-chunked datasets.
    ...
    # Using pre-chunked (.pt + manifest) dataset
    python chronos.py finetune \
        --pre_pt_dataset \
        --model-path "./my_chronos_model" \
...
| `--pre_pt_dataset`      | `train`, `finetune`                 | Load pre-chunked consolidated `.pt` tensor dataset from directory specified in `--train` (requires `--max_length`). Mutually exclusive with `--pre_chunked_dataset`. | `False`           |
...
### v0.6.2 (alpha)
- **Migrated from keyboard to signal**:
      - now uses pythn standard "signal" library for chat interruption due to weird root requirements in the "keyboard" library.


### v0.6.1 (alpha)

  - **Optimized Pre-Chunked Tensor Loading (`--pre_pt_dataset`)**:
      - `dataset_chunk_create.py` now saves **consolidated `.pt` files**, each containing multiple chunks (configurable via `--chunks-per-file`), significantly reducing the number of files generated.
      - A `manifest.jsonl` file is created alongside the consolidated files, mapping each logical chunk to its file and index within that file.
      - `PTChunkedDataset` in `chronos.py` was updated to read the manifest and load chunks from the consolidated files.
      - Implemented **caching** within `PTChunkedDataset` to avoid re-reading the same consolidated `.pt` file when accessing consecutive chunks, improving I/O efficiency during training.
  - **Documentation**: Updated README to reflect the consolidated chunking workflow and caching mechanism. Added `--chunks-per-file` argument to documentation.
