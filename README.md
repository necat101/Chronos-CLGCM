...
# Custom Liscense in Liscence.md
[![Python Version] 3.8+
...
* **High-Performance Inference:** Utilizes a custom C++ kernel inspired by `llama.cpp` for state-of-the-art quantization (`INT4`, `Q8_0`, `Q2_K`, and more).
...
* **Comprehensive Tooling:** Includes a single, powerful script for:
    ...
    * Interactive chat with optional online learning
...
To train the model to produce a "thought process" before its final response, use **Kayla Mode**. This requires a dataset with three fields: `Instruction`, `thought-process`, and `output`.

**Example `kayla_data.jsonl` entry:**
```json
{"Instruction": "Solve for x in 2x + 5 = 11.", "thought-process": "First, I need to isolate the term with x. I will subtract 5 from both sides. This gives me 2x = 6. Then, I need to solve for x by dividing both sides by 2. This results in x = 3.", "output": "The solution is x = 3."}
```
...
Convert your trained `.pt` model to a much smaller and faster `.npz` file. **Remember to provide the same architectural flags used during training!**

```sh
python chronos.py quantize --ckpt "./my_model/chronos_final.pt" --qtype INT4 --context_dim 512 --max_length 2048
```
...
Run an interactive chat session with your model.

#### Running the Quantized Model (Recommended)

* **On CPU:**
    ```sh
    python chronos.py chat --load-quantized "./my_model/chronos_final-INT4.npz" --context_dim 512 --max_length 2048
    ```
* **On GPU (Vulkan):**
    ```sh
    python chronos.py chat --load-quantized "./my_model/chronos_final-INT4.npz" --context_dim 512 --max_length 2048 --device vulkan
    ```
...
* **Modify Base Weights:** Updates are applied directly in memory (lost when you close the app).
    ```sh
    python chronos.py chat --load-quantized "./my_model/chronos_final-INT4.npz" --context_dim 512 --enable-quantized-learning --ckpt "./my_model/chronos_final.pt"
    ```
    *Note: `--enable-quantized-learning` requires providing the original `--ckpt` to use as a "shadow model" for calculating gradients.*

* **Save Updates Separately:** To save the LTM updates permanently without modifying the base model, use `--ltm-lora-path`.
    ```sh
    python chronos.py chat --load-quantized "./my_model/chronos_final-INT4.npz" --context_dim 512 --enable-quantized-learning --ckpt "./my_model/chronos_final.pt" --ltm-lora-path "./my_ltm_updates.pt"
    ```
    The updates will be saved to `my_ltm_updates.pt` when you quit the chat, and automatically loaded the next time you run the command.
...
| `--ckpt`| Path to a full-precision `.pt` model for quantization or chat. | `None` |
...
please donate to my patreon to help with my funding situation, I have motor cortex damage and cant get a job at a datacenter and I currently work at a grocery store full time and work on this on the side: https://www.patreon.com/cw/MakhiBurroughs
