#!/usr/bin/env python3
"""Verify raw residual -> LN1 -> time-mix -> DeepEmbed/channel-mix Vulkan parity."""
...
def main() -> None:
    ...
    x = (torch.randn(batch, width, dtype=torch.float32) * 0.25).requires_grad_()
    previous_tm = (torch.randn(batch, width, dtype=torch.float32) * 0.22).requires_grad_()
    previous_cm = (torch.randn(batch, width, dtype=torch.float32) * 0.2).requires_grad_()
    matrix_state = (
        torch.randn(batch, heads, head_size, head_size, dtype=torch.float32) * 0.08
    ).requires_grad_()
    token_features = (
        torch.randn(batch, input_dim, dtype=torch.float32) * 0.3
    ).requires_grad_()
    ...
    def matrix(rows: int, cols: int, scale: float = 0.18) -> torch.Tensor:
        return (
            torch.randn(rows, cols, dtype=torch.float32) * (scale / math.sqrt(max(1, rows)))
        ).requires_grad_()
    ...
    x_norm = F.layer_norm(
        x, (width,), weight=ln1_weight, bias=ln1_bias, eps=1.0e-5
    )
    ...
    time_branch = F.linear((group_normed + bonus) * g, output_weight)
    x_after_time = x + time_branch
    ...
    ffn = ffn * torch.clamp(deepembed, -4.0, 4.0)
    ffn = torch.clamp(ffn, -(12.0 * 12.0 * 4.0), 12.0 * 12.0 * 4.0)
    output = x_after_time + F.linear(ffn, value_cm_weight)

    grad_matrix_state_out = torch.randn_like(new_matrix_state) * 0.018
    grad_output = torch.randn_like(output) * 0.035
    objective = (
        (new_matrix_state * grad_matrix_state_out).sum()
    ...
    case = {
        ...
        "previous_tm": previous_tm.detach().flatten().tolist(),
        "previous_cm": previous_cm.detach().flatten().tolist(),
        "matrix_state": matrix_state.detach().flatten().tolist(),
        "token_features": token_features.detach().flatten().tolist(),
        "grad_matrix_state_out": grad_matrix_state_out.flatten().tolist(),
        "grad_output": grad_output.flatten().tolist(),
    }

    with tempfile.TemporaryDirectory(prefix="hierarchos-vulkan-rwkv-cell-") as temp_dir:
        temp = Path(temp_dir)
        ...
        completed = subprocess.run(
            [
                ...
                "--bin",
                "hierarchos-vulkan-rwkv-cell-slice-step",
                "--",
                "--case",
                str(case_path),
                "--model-dir",
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "Vulkan RWKV cell-slice parity runner failed:\n"
                f"stdout:\n{completed.stdout}\nstderr:\n{completed.stderr}"
            )
    ...
    comparisons = {
        ...
        "new_matrix_state": (result["new_matrix_state"], new_matrix_state.detach()),
        "grad_x": (result["grad_x"], x.grad),
        "grad_matrix_state": (result["grad_matrix_state"], matrix_state.grad),
        "grad_previous_tm": (result["grad_previous_tm"], previous_tm.grad),
        "grad_previous_cm": (result["grad_previous_cm"], previous_cm.grad),
        "token_feature_grad": (result["token_feature_grad"], token_features.grad),
        "grad_ln1_weight": (result["grad_ln1_weight"], ln1_weight.grad),
        "grad_ln1_bias": (result["grad_ln1_bias"], ln1_bias.grad),
        "grad_channel_mix_k": (result["grad_channel_mix_k"], mix_k_cm.grad),
        "grad_channel_key_weight": (result["grad_channel_key_weight"], key_cm_weight.grad),
        "grad_channel_value_weight": (result["grad_channel_value_weight"], value_cm_weight.grad),
        "grad_ln2_weight": (result["grad_ln2_weight"], ln2_weight.grad),
        "grad_ln2_bias": (result["grad_ln2_bias"], ln2_bias.grad),
    }
    ...
    for name, (actual_values, expected) in comparisons.items():
        ...
        torch.testing.assert_close(actual, expected, rtol=1.5e-3, atol=1.5e-5)
        diffs[name] = (actual - expected).abs().max().item()

    print(
        f"device={result['device']} width={result['width']} head_size={result['head_size']}"
    )
    print(" ".join(f"max_abs_{name}={value:.9g}" for name, value in diffs.items()))
    print("Hierarchos Vulkan fused raw RWKV cell slice PyTorch parity: PASS")
